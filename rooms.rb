...
#Find all the events taking place in room 201. Iterate through them and print "ok" if the number of planned attendees
#will fit in the room.

all[:data][:events].each do |event|
  if event.room_id == 1
    if
      event.attendees < all.data.rooms[0].capacity
    end
    puts "ok"
  elsif
